...
class Shape_Drawer:
    ...
    def _transform_2_iterations(self):
        pt_last = self._coordinates[0]
        for idx in range(len(self._coordinates)):
            step_y_U = self._coordinates[idx][1] - pt_last[1]
            step_y_MM = step_y_U * self.CONVERSION_U_2_MM

            step_x_U = self._coordinates[idx][0] - pt_last[0]
            step_x_MM = step_x_U * self.CONVERSION_U_2_MM
            
            self.iterations.append([int(round(step_x_MM * self.CONVERSION_MM_2_IT)), int(round(step_y_MM * self.CONVERSION_MM_2_IT))])

            pt_last = self._coordinates[idx]

circle = Shape_Drawer()
pubX = rospy.Publisher("stepperCommandX", Int16, queue_size=10)
pubY = rospy.Publisher("stepperCommandY", Int16, queue_size=10)
status_ready = True
arduino_ready = False
...
def stepperControlCommand():
    ...
    while not rospy.is_shutdown():
        if arduino_ready and status_ready:
            ...
            pubX.publish(Int16(it[0]))
            pubY.publish(Int16(it[1]))
            status_ready = False
